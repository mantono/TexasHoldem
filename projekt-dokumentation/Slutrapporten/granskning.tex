...
\item[cards] Card / Colour / Deck / Rank
	\begin{description}
		\item[Rank:14] Value och rank missförstånd kan uppstå mellan variabelnamnet och metodnamnet.
		\item[Rank:5] Funderingar över hur vi ska lösa hantering av att Rank.ACE kan vara 1 i enum-klassen Rank.java. Detta sågs inte vara något allvarligt och bestämdes att det skulle lösas direkt där det behövdes, t ex vid uträkning av kombinationen CardCombination.STRAIGHT.
		\item[Colour:13] Enum-klassen Colour går nu att använda en getValue metod på även fast vi är osäkra på hur ranking mellan olika färger går till i Texas Hold Em och andra kortspel. Detta anses ganska allvarligt då det kan leda till felaktiga uträkningar i vinnande händer osv.
		\item[Colour:13, Rank:14] getValue-metoden har skyddsnivån public i enum-klassen Colour men protected i enum-klassen Rank vilket, den borde ha samma skyddsnivå i båda klasserna vilket är något som ska korrigeras.
		\item[Deck:11-22] Byta plats på konstruktorerna i klassen Deck för bättre läsbarhet.
	\end{description}
\item[texasholdem] Action / Game / Player / Pot
	\begin{description}
		\item[Player:33] Implementation av clearhand() i Game-klassen istället för player?
		\item[Player:41] Returtyp int på addChips(), behövs det?
		\item[Player:45]Ska addToPot returnera en boolean? Exception vid i>chips?
		\item[Pot:12] If-satsen i konstruktorn som tar emot en parameter för klassen Pot måste ändras så att du kan skapa en pot med 0 chips i, för tillfället så får man ett exception då vilket inte är meningen. Detta ses som relativt allvarligt då inget ska hindra dig från att skapa en pot med 0 chips i.
		\item[Pot:9] betHistory i Pot borde sättas till final då det inte ska skapas flera av den utan bara tömma den redan skapade mapen.
		\item[Pot:28] Metoden checkNull i Pot visar sig vara överflödig och ska tas bort.
		\item[Pot:43] betToPot-metoden borde byta namn till någon som bättre representerar dess funktion, t ex recieveBet.
		\item[Player:60] Metoden getInGame i klassen Player borde borde byta namn till isInGame för att bättre representera sin funktion.
		\item[Player:45] addToPot-metoden borde byta namn för att bättre representera sin funktion eller göras om så att mer av denna funktionalitet ligger i Game-klassen och det enda som görs i den här metoden är att ta bort de chipsen som spelare bettar.
		\item[Player:45] Byte av varibelnamn i metoden addToPot från i till bet för att öka läsbarheten.
	\end{description}
\end{description}
